...
# INTERNAL LIBRARY
####################################
from source.utilities import Get_Logger, Average_Meter, augment_xy_data_by_16_fold_2obj_t2
...
class Meta(nn.Module):
    ...
    def test(self, model=None, testdata=None, weight=None):
        if model is None:
            ...
            test_model = model
        if testdata is None:
            testdata = torch.FloatTensor(np.random.rand(TEST_BATCH_SIZE, TSP_SIZE, 3))
            testdata = torch.cat((testdata, torch.zeros([TEST_BATCH_SIZE, TSP_SIZE, 1])), dim=-1)
        if weight is None:
            weight = torch.tensor([0.5, 0.5])
    ...
    def test_aug(self, model=None, testdata=None, weight=None):
        ...
        testdata_aug = augment_xy_data_by_16_fold_2obj_t2(testdata)
        testdata_aug = testdata_aug.reshape(16, -1, TSP_SIZE, 4).permute(1, 0, 2, 3).reshape(-1, TSP_SIZE, 4)
        ...
        while True:
            ...
            remaining = testdata.size(0) - episode
            batch_s = min(TESTAUG_BATCH_SIZE, remaining) * 16
            testdata_batch = testdata_aug[episode * 16: episode * 16 + batch_s]
            ...
            with torch.no_grad():
                ...
                # RECORDING
                ###############################################
                agg_reward = agg_reward.reshape(-1, 16 * group_s)
                reward = reward.reshape(-1, 16 * group_s, 2)

                min_agg_obj, min_i = agg_reward.min(dim=1)
                min_i = min_i[:, None, None].repeat(1, 1, 2)
                min_obj = reward.gather(1, min_i)
                all_min_obj[episode: episode + batch_s // 16] = torch.squeeze(min_obj, 1)

            episode = episode + batch_s // 16
            if episode == TEST_DATASET_SIZE:
                break
...
if MODE == 1:  # Train
    ...
    testdata = torch.load('../test_tspt2/testdata_tspt2_size{}.pt'.format(TSP_SIZE))
