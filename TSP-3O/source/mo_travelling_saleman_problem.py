...
class TSP_Dataset__Random(Dataset):
    ...
    def __getitem__(self, index):
        node_xy_data = np.random.rand(self.num_nodes, 6)
        # node_xy_data = np.random.rand(self.num_nodes, 6) / np.array([1., 1., 2., 2., 10., 10.])
        # node_xy_data = np.random.rand(self.num_nodes, 6) / np.array([1., 1., 2., 2., 4., 4.])
