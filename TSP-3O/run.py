...
# INTERNAL LIBRARY
####################################
from source.utilities import Get_Logger, Average_Meter, augment_xy_data_by_n_fold_3obj
...
class Meta(nn.Module):
    ...
    def forward(self, meta_lr=None, f=None):
        ...
        for data in tqdm(support_loader):
            ...
            ###############################################
            reward = - reward
            reward = reward.reshape(self.task_num, TRAIN_BATCH_SIZE, TSP_SIZE, OBJ_NUM)
            assert AGG == 1 or AGG == 2 or AGG == 3 or AGG == 4, "Only support Weighted-Sum and Weighted-Tchebycheff and PBI and IPBI"
            if AGG == 1:
                ...
                d1 = (ww * (reward - z)).sum(dim=-1) / torch.norm(ww)
                d1_variant = ww.reshape(self.task_num, OBJ_NUM, 1, 1) * d1
                d1_varaint = rearrange(d1_variant, 'a c b h -> a b h c')
                d2 = torch.norm(reward - z - d1_varaint)
                ...
                d1 = (ww * (z - reward)).sum(dim=-1) / torch.norm(ww)
                d1_variant = ww.reshape(self.task_num, OBJ_NUM, 1, 1) / torch.norm(ww) * d1
                d1_varaint = rearrange(d1_variant, 'a c b h -> a b h c')
    ...
    def test_aug(self, model=None, testdata=None, weight=None):
        ...
        testdata_aug = augment_xy_data_by_n_fold_3obj(testdata, AUG_NUM)
...
if MODE == 1:  # Train
    ...
    for j in range(1, max_i):
        ...
        for wk, wv in hw[j].items():
            wv_adjust = [0 for _ in range(3)]
            wv_i = [0 for _ in range(3)]
            op = 0
            for i, wi in enumerate(wv[:2]):
                if (wi * pre_part) in range(1, pre_part):
                    if op % 2 == 0:
                        wv_adjust[i] = wi - 10e-6
                        wv_i[i] = int((wi - 10e-6) * pre_part)
                    else:
                        wv_adjust[i] = wi + 10e-6
                        wv_i[i] = int((wi + 10e-6) * pre_part)
                    op += 1
            ...
            wv_adjust[2] = 1 - wv_adjust[0] - wv_adjust[1]
            if wv_adjust[2] == 1:
                wv_i[2] = int((wv_adjust[2] - 10e-6) * pre_part)
            else:
                wv_i[2] = int(wv_adjust[2] * pre_part)
    ...
    if n_sols == 105:
        ...
        uniform_weights = torch.Tensor(das_dennis(23, 3))  # 300
    ...
    for i in range(n_weight):
        ...
        wv_adjust = [0 for _ in range(3)]
        wv_i = [0 for _ in range(3)]
        for ind, wi in enumerate(weight[:2]):
            op = 0
            if (wi * pre_part) in range(1, pre_part):
                if op % 2 == 0:
                    wv_adjust[ind] = wi - 10e-6 / pre_part
    ...
    p_sols, _, p_sols_num = update_EP(torch.tensor(all_sols).float(), None)
    hvs = cal_ps_hv(pf=p_sols, pf_num=p_sols_num, ref=ref)
    p_sols_aug, _, p_sols_num_aug = update_EP(torch.tensor(all_sols_aug).float(), None)
    hvs_aug = cal_ps_hv(pf=p_sols_aug, pf_num=p_sols_num_aug, ref=ref)

    print('Test Time(s): {:.4f}'.format(run_time))
    print('HV Ratio: {:.4f}'.format(hvs.mean()))
    print('NDS: {:.4f}'.format(p_sols_num.float().mean().item()))
    print('Aug Test Time(s): {:.4f}'.format(run_time_aug))
    ...
    file.write('Test Time(s): ' + str(run_time) + '\n')
    file.write('Aug HV Ratio: ' + str(hvs_aug.mean()) + '\n')
    file.write('Aug NDS: ' + str(p_sols_num_aug.float().mean().item()) + '\n')
    file.write('Aug Test Time(s): ' + str(run_time_aug) + '\n')
