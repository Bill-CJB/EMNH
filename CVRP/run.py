...
# INTERNAL LIBRARY
####################################
from source.utilities import Get_Logger, Average_Meter, augment_xy_data_by_8_fold
...
class Meta(nn.Module):
    ...
    def forward(self, meta_lr=None, f=None):
        if f is None:
            f = torch.tensor([1., 1.])
        support_loader = CVRP_DATA_LOADER__RANDOM(num_sample=TRAIN_BATCH_SIZE * UPDATE_STEP, num_nodes=PROBLEM_SIZE, batch_size=TRAIN_BATCH_SIZE)
        ...
        self.modelpl.train()
        for depot_xy, node_xy, node_demand in tqdm(support_loader):
            # depot_xy.shape = (batch, 1, 2)
            # node_xy.shape = (batch, problem, 2)
            # node_demand.shape = (batch, problem, 1)
            # parallel data
            depot_xy = depot_xy.repeat(self.task_num, 1, 1)
            node_xy = node_xy.repeat(self.task_num, 1, 1)
            node_demand = node_demand.repeat(self.task_num, 1, 1)
    ...
    def finetune(self, weight=None, finetune_loader=None, model=None):
        ...
        if finetune_loader is None:
            finetune_loader = CVRP_DATA_LOADER__RANDOM(num_sample=TRAIN_BATCH_SIZE * FINETUNE_STEP, num_nodes=PROBLEM_SIZE, batch_size=TRAIN_BATCH_SIZE)
        if model is None:
        ...
        step = 0
        for depot_xy, node_xy, node_demand in finetune_loader:
            step += 1
            # depot_xy.shape = (batch, 1, 2)
            # node_xy.shape = (batch, problem, 2)
            # node_demand.shape = (batch, problem, 1)
    ...
    def validate(self, model=None, validate_loader=None, weight=None):
        ...
        if validate_loader is None:
            validate_loader = CVRP_DATA_LOADER__RANDOM(num_sample=TRAIN_BATCH_SIZE, num_nodes=PROBLEM_SIZE, batch_size=TRAIN_BATCH_SIZE)
        if weight is None:
        ...
        obj1 = Average_Meter()
        valid_model.eval()
        for depot_xy, node_xy, node_demand in validate_loader:
            # depot_xy.shape = (batch, 1, 2)
            # node_xy.shape = (batch, problem, 2)
            # node_demand.shape = (batch, problem, 1)
            batch_s = depot_xy.size(0)
    ...
    def test(self, model=None, testdata=None, weight=None):
        if model is None:
            ...
            test_model = model
        if testdata is None:
            testdata = torch.load('testdata_size{}.pt'.format(PROBLEM_SIZE))
        if weight is None:
            weight = torch.tensor([0.5, 0.5])
        ...
        depot_xy = testdata['node_data'][:, [0], :].cuda()
        node_xy = testdata['node_data'][:, 1:, :].cuda()
        node_demand = testdata['demand_data'].cuda()
        episode = 0
        while True:

            # seq = Tensor(np.random.rand(TEST_BATCH_SIZE, TSP_SIZE, 2))

            remaining = depot_xy.size(0) - episode
            batch_s = min(TEST_BATCH_SIZE, remaining)
            depot_xy_batch = depot_xy[episode: episode + batch_s]
            node_xy_batch = node_xy[episode: episode + batch_s]
            node_demand_batch = node_demand[episode: episode + batch_s]

            with torch.no_grad():

                env = GROUP_ENVIRONMENT(depot_xy_batch, node_xy_batch, node_demand_batch)
                group_s = PROBLEM_SIZE
                group_state, reward, done = env.reset(group_size=group_s)
    ...
    def test_aug(self, model=None, testdata=None, weight=None):
        ...
        aug_depot_xy = aug_depot_xy.reshape(8, -1, 1, 2).permute(1, 0, 2, 3).reshape(-1, 1, 2)
        aug_node_xy = augment_xy_data_by_8_fold(node_xy)
        # aug_node_xy.shape = (8*batch, problem, 2)
        aug_node_xy = aug_node_xy.reshape(8, -1, PROBLEM_SIZE, 2).permute(1, 0, 2, 3).reshape(-1, PROBLEM_SIZE, 2)
        aug_node_demand = node_demand.repeat(8, 1, 1)
        # aug_node_demand.shape = (8*batch, problem, 2)
        aug_node_demand = aug_node_demand.reshape(8, -1, PROBLEM_SIZE, 1).permute(1, 0, 2, 3).reshape(-1, PROBLEM_SIZE, 1)
        ...
        while True:
            ...
            # batch_s = testdata.size(0)
            # episode = episode + batch_s
            remaining = depot_xy.size(0) - episode
            batch_s = min(TESTAUG_BATCH_SIZE, remaining) * 8
            depot_xy_batch = aug_depot_xy[episode * 8: episode * 8 + batch_s]
            node_xy_batch = aug_node_xy[episode * 8: episode * 8 + batch_s]
            node_demand_batch = aug_node_demand[episode * 8: episode * 8 + batch_s]
            ...
            with torch.no_grad():
                ...
                while not done:
                    action_probs = test_model_aug.get_action_probabilities(group_state)
                    # shape = (batch, group, problem+1)
                    action = action_probs.argmax(dim=2)
                    # shape = (batch, group)
                    action[group_state.finished] = 0  # stay at depot, if you are finished
                ...
                # RECORDING
                ###############################################
                agg_reward = agg_reward.reshape(-1, 8 * group_s)
                reward = reward.reshape(-1, 8 * group_s, 2)

                min_agg_obj, min_i = agg_reward.min(dim=1)
                min_i = min_i[:, None, None].repeat(1, 1, 2)
                min_obj = reward.gather(1, min_i)
                all_min_obj[episode: episode + batch_s // 8] = torch.squeeze(min_obj, 1)
...
if MODE == 1:  # Train
    ...
    tb_logger = TbLogger('logs/CVRP_' + METHOD + '_n{}_{}'.format(PROBLEM_SIZE, time.strftime("%Y%m%dT%H%M%S")))
    ...
    testdata = torch.load('../test_cvrp/testdata_cvrp_size{}.pt'.format(PROBLEM_SIZE))
    ...
    for j in range(1, 7):
        ...
        finetune_loader = CVRP_DATA_LOADER__RANDOM(num_sample=TRAIN_BATCH_SIZE * k, num_nodes=PROBLEM_SIZE,
    ...
    np.savetxt(os.path.join(test_save_dir, "mean_sols_aug.txt"), np.mean(all_sols_aug, axis=0), fmt='%1.4f\t%1.4f', delimiter='\t')
    print(MODEL_DIR)
    ...
    # start_epoch = checkpoint['epoch']
    print('Loaded meta-model-{}'.format(test_ep))
    finetune_loader = CVRP_DATA_LOADER__RANDOM(num_sample=TRAIN_BATCH_SIZE * FINETUNE_STEP, num_nodes=PROBLEM_SIZE,
                                               batch_size=TRAIN_BATCH_SIZE)
    sols = np.empty((n_weight, TEST_DATASET_SIZE, 2))
    sols_aug = np.empty((n_weight, TEST_DATASET_SIZE, 2))
    ...
    hv_ratio_aug = np.empty((TEST_DATASET_SIZE))
    for i in range(TEST_DATASET_SIZE):
        os.makedirs(os.path.join(test_save_dir, "data{}".format(i)))
        os.makedirs(os.path.join(test_save_dir, "data{}_aug".format(i)))
        np.savetxt(os.path.join(test_save_dir, "sols", "ins{}.txt".format(i)), all_sols[i],
                   fmt='%1.4f\t%1.4f', delimiter='\t')
        np.savetxt(os.path.join(test_save_dir, "sols_aug", "ins{}.txt".format(i)), all_sols_aug[i],
                   fmt='%1.4f\t%1.4f', delimiter='\t')
